...
export function useFlashDetector({ videoRef, enabled, targetColor, threshold }: FlashDetectorConfig) {
    ...
    useEffect(() => {
        ...
        const checkFrame = () => {
            if (!enabled || !targetColor || !videoRef.current || videoRef.current.readyState !== 4) {
