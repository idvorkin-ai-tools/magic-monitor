...
```bash
surge token | gh secret set SURGE_TOKEN --repo idvorkin/magic-monitor --env surge-deploy
```
...
- **Live Site**: Visit https://magic-monitor.surge.sh
...
- Pull requests to `main` → deploys to `pr-N-magic-monitor.surge.sh`
